...
This exercise will use unpublished (!) RNA Seq data (gene counts) from cultures of *Saccharomyces cerevisiae* of mating type **a** that have been externally treated with a pheromone called 'alpha factor'. In the wild, haploid yeast cells mate via the production of pheromones (Figure 1), which signals their presence to neighboring cells. When cells of the opposite mating type sense each other (via their pheromones) they respond by forming 'shmoos', growth projections that allow cells to make contact and subsequently mate to form a diploid cell. The aim of this work was therefore to find out how pheromone treatment affects cells, using a genome-wide perspective.

```{r pressure, echo=FALSE, fig.cap="**Figure 1** Adapted from: https://en.wikipedia.org/wiki/Mating_of_yeast", out.width = '75%', fig.align='center'}
knitr::include_graphics("Yeast_mating_scheme.jpg")
```
...
> **Question `r qn<-qn+1;qn`:** Take a closer look at these genes (you can use https://www.yeastgenome.org/ for help). Do you see anything interesting related to their functions?
...
For a brief recap on why FDR adjustment is important, during multiple testing, see here: https://www.explainxkcd.com/wiki/index.php/882:_Significant.
...
`r sn<-sn+1; paste(sn,". ", sep="")` Export the results table to a .txt file:
```{r, eval=T}
write.table(qlf$table, file = 'data/DE_results/DE_results_merged.txt', quote=F, sep='\t')
```
