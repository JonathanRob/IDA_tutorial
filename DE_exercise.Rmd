...
This exercise will use unpublished (!) RNA-Seq data (gene counts) from cultures of *Saccharomyces cerevisiae* with and without pheromone treatment. The count data files are located in the `data/gene_counts/` subdirectory in the `IDA_tutorial` directory.

The `gene_counts_glucose.txt` dataset corresponds to glucose as the carbon source, whereas the `gene_counts_ethanol.txt` dataset used ethanol. Each dataset includes 6 samples after pheromone treatment, and several samples (24 for glucose, 15 for ethanol) without treatment.
...
<div class="answer"><table class="answer"><tr><td class="answer">
**Answer:** 
Yes, the untreated (U) and pheromone-treated (P) samples clearly separate on the x-axis, indicating a substantial difference in transcriptomic signatures between the two conditions.
...
It looks like there may be some further grouping among the untreated samples...
</td></tr></table></div>
