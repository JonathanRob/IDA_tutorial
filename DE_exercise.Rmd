...
This exercise will use unpublished RNA-Seq data (gene counts) from cultures of *Saccharomyces cerevisiae* before and after pheromone treatment, and should be in the `data/` subdirectory in your R working directory. 
...
> **Question `r qn<-qn+1;qn`:** Blah blah blah?
...
We want to determine genes that are differentially expressed between two groups - in this case, between "untreated" and "pheromone-treated" samples, which are indicated by column names beginning with a "U" or "P", respectively. We can take a quick look at the column names to see what samples we have:
```{r}
colnames(count.data)
```
...
```{r, eval=F}
write.table(res.table, file = 'DE_results_glucose.txt', quote=F, sep='\t')
```
