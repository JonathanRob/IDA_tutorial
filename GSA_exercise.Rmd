...
The GSEA method we used previously takes only one set of values for the genes, so we had to combine our fold-changes and p-values into a single metric. For methods such as Stouffer's, we provide the gene p-values **and** the fold-change direction (`sign(logfc)`) as separate inputs. This allows Piano to calculate additional directionality classes (distinct directional, mixed-directional, and non-directional). If you forgot what these mean, check out the <a href="https://dx.doi.org/10.1093%2Fnar%2Fgkt111" target="_blank">Piano paper</a>.
